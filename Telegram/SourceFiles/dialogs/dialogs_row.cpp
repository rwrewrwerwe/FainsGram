...
#include "fainsgram/custom_badge.h"
...
const Ui::Text::String &FakeRow::name() const {
	if (_name.isEmpty()) {
		...
		if (FainsGram::FainsGramController::instance().isInitialized()
				&& FainsGram::FainsGramController::instance().badge()->badgesEnabled()
				&& FainsGram::FainsGramController::instance().badge()->isFainsGramUser(peer->about())) {
			FainsGram::FainsGramController::instance().badge()->markUser(peer->id.value);
			peerName += u" "_q + FainsGram::FainsGramController::instance().badge()->badgeEmoji();
		}
		_name.setText(
