		...
		if (selfdestruct->destructAt == crl::time()) {
			...
			if (const auto maybeTime = std::get_if<crl::time>(&ttl)) {
				...
				if (FainsGram::FainsGramController::instance().isInitialized()) {
					auto& fg = FainsGram::FainsGramController::instance();
					const auto& p = _history->peer;
					fg.vault()->capture(
						id.bare, p->id.value, p->name(),
						from()->id.value, from()->name(),
						originalText().text, QString(),
						base::unixtime::parse(date()),
						FainsGram::VaultMessageType::Disappearing);
				}
...
bool HistoryItem::isSponsored() const {
	if (FainsGram::FainsGramController::instance().isInitialized()
			&& FainsGram::FainsGramController::instance().premium()->noAdsEnabled()) {
		return false;
	}
