...
void BottomInfo::layoutDateText() {
	...
	auto date = edited + QLocale().toString(
		_data.date.time(),
		QLocale::ShortFormat);

	if (_data.ttlSeconds > 0) {
		const auto ttlStr = (_data.ttlSeconds > 86400 || _data.ttlSeconds == 2147483647)
			? QString("one view, ")
			: (QString::number(_data.ttlSeconds) + "s, ");
		date = ttlStr + date;
	}
	if (_data.isDeleted) {
		date = u"del \u2014 "_q + date;
...
BottomInfo::Data BottomInfoDataFromMessage(not_null<Message*> message) {
	...
	if (const auto media = item->media()) {
		result.ttlSeconds = media->ttlSeconds();
	}
	result.isDeleted = item->isDeletedPreserved();
