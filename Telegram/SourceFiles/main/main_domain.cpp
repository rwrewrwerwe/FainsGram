...
#include "fainsgram/multi_account_manager.h"
...
not_null<Main::Account*> Domain::add(MTP::Environment environment) {
	Expects(started());
	const auto limit = (FainsGram::FainsGramController::instance().isInitialized()
		&& FainsGram::FainsGramController::instance().accounts()->isUnlimitedEnabled())
		? FainsGram::FainsGramController::instance().accounts()->effectiveMaxAccounts()
		: kPremiumMaxAccounts;
	Expects((int)_accounts.size() < limit);
