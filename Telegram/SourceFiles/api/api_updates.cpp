...
#include "fainsgram/ghost_controller.h"
	...
	if (isOnline != _lastWasOnline
		...
		if (!Core::Quitting()) {
			if (FainsGram::FainsGramController::instance().isInitialized()
					&& FainsGram::FainsGramController::instance().ghost()->shouldSuppressUpdateStatus()) {
				// Ghost Mode: don't send online status to server
			} else {
