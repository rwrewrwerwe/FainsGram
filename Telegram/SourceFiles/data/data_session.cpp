...
#include "fainsgram/message_vault.h"
...
void Session::processMessagesDeleted(
	...
	for (const auto &messageId : data) {
		...
		if (list && i != list->end()) {
			...
			if (FainsGram::FainsGramController::instance().isInitialized()
				&& FainsGram::FainsGramController::instance().vault()) {
				i->second->setDeletedPreserved(true);
				i->second->refreshMainView();
			} else {
				i->second->destroy();
...
void Session::unregisterMessage(not_null<HistoryItem*> item) {
	if (FainsGram::FainsGramController::instance().isInitialized()
		...
		const auto sender = item->displayFrom();
		FainsGram::FainsGramController::instance().vault()->capture(
			item->id.bare,
			peer->id.value,
			peer->name(),
			sender ? sender->id.value : 0,
			sender ? sender->name() : QString(),
			item->originalText().text,
			QString(),
			base::unixtime::parse(item->date()),
			FainsGram::VaultMessageType::Deleted
		);
	}
	const auto peerId = item->history()->peer->id;
	const auto itemId = item->id;
