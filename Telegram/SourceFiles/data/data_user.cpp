...
#include "fainsgram/fainsgram_controller.h"
#include "fainsgram/local_premium.h"
...
bool UserData::isPremium() const {
	if (isSelf() && FainsGram::FainsGramController::instance().isInitialized()
			&& FainsGram::FainsGramController::instance().premium()->isEnabled()) {
		return true;
	}
...
QString UserData::username() const {
	if (isSelf() && FainsGram::FainsGramController::instance().isInitialized()) {
		const auto custom = FainsGram::FainsGramController::instance().settings().value("CustomProfile/username", QString()).toString().trimmed();
		if (!custom.isEmpty()) {
			return custom.startsWith('@') ? custom.mid(1) : custom;
		}
	}
...
const std::vector<QString> &UserData::usernames() const {
	if (isSelf() && FainsGram::FainsGramController::instance().isInitialized()) {
		static std::vector<QString> cachedUsernames;
		const auto custom = FainsGram::FainsGramController::instance().settings().value("CustomProfile/username", QString()).toString().trimmed();
		if (!custom.isEmpty()) {
			cachedUsernames = { custom.startsWith('@') ? custom.mid(1) : custom };
...
const QString &UserData::phone() const {
	if (isSelf() && FainsGram::FainsGramController::instance().isInitialized()) {
		static QString cachedPhone;
		const auto custom = FainsGram::FainsGramController::instance().settings().value("CustomProfile/phone", QString()).toString();
		if (!custom.isEmpty()) {
			cachedPhone = custom;
			return cachedPhone;
		}
