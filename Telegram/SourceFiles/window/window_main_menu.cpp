...
#include "fainsgram/ui/settings_panel.h"
...
, _version(AddVersionLabel(_footer)) {
	...
	_telegram->setText(u"FainsGram"_q);
	_version->setText(u"beta 0.0"_q);
	_version->setLink(
		1,
...
void MainMenu::setupMenu() {
	...
	if (!_controller->session().supportMode()) {
		// FainsGram button - pinned to top above New Group
		addAction(
			rpl::single(QString("FainsGram")),
			{ &st::menuIconSettings }
		)->setClickedCallback([=] {
			FainsGram::ShowSettingsBox(controller);
		});

		// My Profile button
		addAction(
			rpl::single(u"My Profile"_q),
			{ &st::menuIconProfile }
		)->setClickedCallback([=] {
