...
bool TransferFrame(
	...
	if (transferredFrame->format == AV_PIX_FMT_NONE
		|| transferredFrame->format < 0) {
		if (decodedFrame->hw_frames_ctx && decodedFrame->hw_frames_ctx->data) {
			const auto hwctx = reinterpret_cast<AVHWFramesContext*>(decodedFrame->hw_frames_ctx->data);
			if (hwctx->sw_format != AV_PIX_FMT_NONE) {
				transferredFrame->format = hwctx->sw_format;
			} else {
				transferredFrame->format = AV_PIX_FMT_YUV420P;
			}
		} else {
			transferredFrame->format = AV_PIX_FMT_YUV420P;
		}
	}
	transferredFrame->width = decodedFrame->width;
	transferredFrame->height = decodedFrame->height;
...
QImage ConvertFrame(
	...
	if (frame->format == AV_PIX_FMT_NONE || frame->format == AV_PIX_FMT_VIDEOTOOLBOX || frame->format < 0) {
		if (frame->hw_frames_ctx && frame->hw_frames_ctx->data) {
			const auto hwctx = reinterpret_cast<AVHWFramesContext*>(frame->hw_frames_ctx->data);
			frame->format = (hwctx->sw_format != AV_PIX_FMT_NONE && hwctx->sw_format != AV_PIX_FMT_VIDEOTOOLBOX)
				? hwctx->sw_format
				: AV_PIX_FMT_YUV420P;
		} else {
