...
bool FFMpegReaderImplementation::renderFrame(
	...
	// Handle hardware-accelerated frames (e.g. VideoToolbox on Apple Silicon)
	// by transferring data to a software frame before processing.
	AVFrame *renderFrame = _frame.get();
	FFmpeg::FramePointer swFrame;
	if (_frame->hw_frames_ctx != nullptr
		|| format == AV_PIX_FMT_VIDEOTOOLBOX
		|| format == AV_PIX_FMT_CUDA
		|| format == AV_PIX_FMT_DXVA2_VLD
		|| format == AV_PIX_FMT_VAAPI) {
		swFrame = FFmpeg::MakeFramePointer();
		if (av_hwframe_transfer_data(swFrame.get(), _frame.get(), 0) >= 0) {
			swFrame->width = _frame->width;
			swFrame->height = _frame->height;
			renderFrame = swFrame.get();
		}
	}

	const auto rawFormat = renderFrame->format;
	const auto renderFormat = (rawFormat == AV_PIX_FMT_NONE || rawFormat == AV_PIX_FMT_VIDEOTOOLBOX)
		? ((_frame->hw_frames_ctx && _frame->hw_frames_ctx->data)
			? reinterpret_cast<AVHWFramesContext*>(_frame->hw_frames_ctx->data)->sw_format
			: ((_codecContext->pix_fmt != AV_PIX_FMT_NONE && _codecContext->pix_fmt != AV_PIX_FMT_VIDEOTOOLBOX)
				? _codecContext->pix_fmt
				: AV_PIX_FMT_YUV420P))
		: rawFormat;
	const auto bgra = (renderFormat == AV_PIX_FMT_BGRA);
	hasAlpha = bgra || (renderFormat == AV_PIX_FMT_YUVA420P);
	if (renderFrame->width == toSize.width()
		&& renderFrame->height == toSize.height()
		&& bgra) {
		int32 sbpl = renderFrame->linesize[0], dbpl = to.bytesPerLine(), bpl = qMin(sbpl, dbpl);
		uchar *s = renderFrame->data[0], *d = to.bits();
		for (int32 i = 0, l = renderFrame->height; i < l; ++i) {
			memcpy(d + i * dbpl, s + i * sbpl, bpl);
		}
	} else {
		if ((_swsSize != toSize)
			|| (_swsFormat != int(renderFormat))
			|| !_swsContext) {
			_swsSize = toSize;
			_swsFormat = int(renderFormat);
			_swsContext = sws_getCachedContext(
				_swsContext,
				renderFrame->width,
				renderFrame->height,
