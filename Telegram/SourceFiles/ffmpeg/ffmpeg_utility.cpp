...
#ifdef Q_OS_MAC
	// On macOS, VideoToolbox hwaccel frame transfer via av_hwframe_transfer_data
	// causes playback failures and color distortion. Software decoders with ARM NEON
	// assembly are extremely fast and 100% reliable for all video formats.
...
SwscalePointer MakeSwscalePointer(
	...
	if (format == AV_PIX_FMT_NONE || format == AV_PIX_FMT_VIDEOTOOLBOX || format < 0) {
		if (frame->hw_frames_ctx && frame->hw_frames_ctx->data) {
			...
			format = (hwctx->sw_format != AV_PIX_FMT_NONE && hwctx->sw_format != AV_PIX_FMT_VIDEOTOOLBOX)
				? hwctx->sw_format
				: AV_PIX_FMT_YUV420P;
		} else {
...
int ReadRotationFromMetadata(not_null<AVStream*> stream) {
	...
	const auto displaymatrix = _sd ? _sd->data : nullptr;
	auto theta = 0;
	if (displaymatrix) {
		theta = -round(av_display_rotation_get((const int32_t*)displaymatrix));
	}
